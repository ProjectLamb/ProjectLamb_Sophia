...
public class CarrierBucket : MonoBehaviour {
    public void CarrierTransformPositionning(Entity _owner, Carrier _carrier, BUCKET_POSITION _type){
    ...
    public GameObject GameObjectInstantiator(Entity _owner, GameObject _go, BUCKET_POSITION _type){
        //재작성하기
        return null;
    }

    public Carrier CarrierInstantiator(Entity _owner, Carrier _carrier, BUCKET_POSITION _type){
        Vector3     position     = transform.position;
        Quaternion  forwardAngle = GetForwardingAngle(_carrier.transform);
    ...
    public Carrier CarrierInstantiatorByObjects(Entity _owner, Carrier _carrier, BUCKET_POSITION _type, object[] _objects){
        Vector3     position     = transform.position;
        Quaternion  forwardAngle = GetForwardingAngle(_carrier.transform);
        Carrier carrierInstant = _carrier.Clone();
