...
public class SkillManager : MonoBehaviour {
    ...
    public ConcreteSkill[] skills = new ConcreteSkill[Enum.GetValues(typeof(E_SkillKey)).Length];
    ...
    private void Start(){
        for(int i = 0; i < 3; i++){
            ...
            skills[i].Initialisze(player, keyByIndex);
        }
    }

    public void AssignSkill(ConcreteSkill _skill, E_SkillKey _key){
        skills[(int)_key] = _skill;
        skills[(int)_key].Initialisze(player, _key);
    }
    public void SwapSkill(E_SkillKey _insert, E_SkillKey _target){
        ...
        skills[(int)_insert].Initialisze(player, _insert);
        skills[(int)_target].Initialisze(player, _target);
    }
