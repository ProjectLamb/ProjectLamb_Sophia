...
namespace Sophia.DataSystem
{
    namespace Modifiers.ConcreteAffectors
    {
        ...
        public class FrozenAffect : Affector
        {
            #region Members
            private CancellationTokenSource cts;
            public Material materialRef { get; private set; }
            ...
            public FrozenAffect(Entity ownerReceivers, Entity targetReceivers, float durateTime)
            : base(E_AFFECT_TYPE.Frozen, ownerReceivers, targetReceivers, durateTime)
            {
                cts = new CancellationTokenSource();
                targetReceivers.GetLifeComposite().OnExitDie += () => cts.Cancel();
            ...
            #endregion
            #region Setter
            public FrozenAffect SetMaterial(Material material)
            {
                this.materialRef = material;
                Timer.OnStart += FrozenMeshOn;
                Timer.OnFinished += FrozenMeshOff;
                return this;
            }
            public FrozenAffect SetModifyData(SerialModifireDatas moveSpeedModifiyData)
            {
                ...
                Timer.OnStart += FrozenStart;
                Timer.OnFinished += FrozenEnd;
                return this;
            }
            #endregion
            ...
            private async void FrozenMeshOn()
            {
                try
                {
                    if (TargetRef == null) return;
            ...
            private async void FrozenMeshOff()
            {
                try
                {
                    if (TargetRef == null) return;
