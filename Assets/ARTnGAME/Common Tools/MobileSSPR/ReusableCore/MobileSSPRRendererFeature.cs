...
namespace Artngame.CommonTools
{
    public class MobileSSPRRendererFeature : ScriptableRendererFeature
    {
        ...
        public class PassSettings
        {
            ...
            //v0.1
            public bool unity2022Compatible = false;
        }
        ...
        public class CustomRenderPass : ScriptableRenderPass
        {
            ...
            {
                //v0.1
                if (renderingData.cameraData.camera != Camera.main || Screen.width <= 0 || Screen.height <= 0)
                { 
                    return;
                }
                ...
                //v0.1
                dispatchThreadGroupXCount = (dispatchThreadGroupXCount > 0) ? dispatchThreadGroupXCount : 1; //NEW
                dispatchThreadGroupYCount = (dispatchThreadGroupYCount > 0) ? dispatchThreadGroupYCount : 1; //NEW
                ...
                if (settings.ShouldRenderSSPR)
                {
                    ...
                    else
                    {
                        ...
                        if (settings.unity2022Compatible)
                        {
                            cb.SetComputeTextureParam(cs, kernel_NonMobilePathRenderHashRT, "_CameraDepthTexture", Shader.GetGlobalTexture("_CameraDepthTexture"));                            
                            cb.SetComputeTextureParam(cs, kernel_NonMobilePathResolveColorRT, "_CameraOpaqueTexture", Shader.GetGlobalTexture("_CameraOpaqueTexture"));
                        }
                        else
                        {
